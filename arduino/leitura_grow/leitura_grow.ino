...
#define WIFI_SSID "Camila e Aline"
#define WIFI_PASSWORD "987877759"

//For the following credentials, see examples/Authentications/SignInAsUser/EmailPassword/EmailPassword.ino

/* 2. Define the API Key */
#define API_KEY "AIzaSyAwPUzvzrQg7d1U0BvQQiLnjrujeeZ129w"

/* 3. Define the RTDB URL */
#define DATABASE_URL "iotboard-ee46a-default-rtdb.firebaseio.com"


/* 4. Define the user Email and password that alreadey registerd or added in your project */
#define USER_EMAIL "cah@gmail.com"
#define USER_PASSWORD "123456"

/* 5. Define the database secret in case we need to access database rules*/
#define DATABASE_SECRET "k11lmJoqqeaVAWKJBlVIdcyBzL70yb5k5W8CVMth"
#define DHTPIN 2
...
const int solo_seco = 632; //Valor com solo seco
const int solo_umido = 314; //Valor com solo umido
...
const int minimo_solo = 30;
const int maximo_solo = 50;
//Altere as variaveis acima com os val
...
void loadDefaultConfig()
{
  ...
  FireSense_Channel channel[3];

  channel[0].id = "umidade";
  channel[0].name = "umidade";
  ...
  cond[2].IF = "SOIL < 340 || SOIL > 640";
  cond[2].THEN = "func(0,1,'\\n[FUNC0] The SOIL ({SOIL} °C) is out of range.\\n')";
  FireSense.addCondition(cond[2]);
}
...
;
  ...
  fsConfig.stream_fbdo = &fbdo3;               // for stream, if set this stream_fbdo to nullptr, the stream will connected through shared FirebaseData object.
  fsConfig.debug = true;

  // Add the user variable that will bind to the channels
  FireSense.addUserValue(&humidity);
  FireSense.addUserValue(&temperature);
  FireSense.addUserValue(&soil);
...
void pump(){
    pinMode(5, OUTPUT);
    concentracao = map(soil, solo_umido, solo_seco, 100, 0);
//check valor para irrigação
//valor se valor for baixo
    if (concentracao < minimo_solo) {
      Serial.print("solo está seco bora irrigar?");
      Serial.println();

     digitalWrite(5, LOW); //Acionamos o rele
      delay(5000); 
       digitalWrite(5, HIGH); //Acionamos o rele
  
     
    }else if ( concentracao > maximo_solo){ //se o val for maior
          Serial.print("solo está bem molhado");
      Serial.println();
          digitalWrite(5, HIGH); //O rele se mantem desligado
